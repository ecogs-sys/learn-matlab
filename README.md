...
| images | [`topics/images`](topics/images) | An image is a **matrix of numbers** — 0 black, 255 white — so `gray + 40` brightens it, `255 - gray` inverts it, and a crop is a block slice. Plus the two silent traps: `uint8` arithmetic saturates at 255 instead of wrapping, and `double(img)` followed by `imshow` gives you a white rectangle with no error at all. |
